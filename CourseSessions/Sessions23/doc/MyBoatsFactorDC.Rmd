...
**Charles Cao, Davide Casarini, Sebastien Vuillod**

Business Decisions
---------------------------------------------------------
We have identified 5 factors that decribe adequately boats owners, these are:
...
Data are organised in a failry traditional fashion, with respondents listed in rows and questions, plus demographic variables listed in columns. While some questions are not necessarily metrics, we have selected only columns 2:29 (we decided to exclude the RESPID column), therefore all data is a metric. Even though there are no variables with values that are several order of magnitude higher than the average of the dataset (like in the MBA Admits analysis), we have performed a standardization of the values.
...
As mentioned above we have decided to standardise the data.
...
This is the correlation matrix of the questions selected as part of the factor analysis. As it can be noted, some questions are correlated with each others and this will form the basis of the factoring process. In other words, the questions with higest correlation among eachoter - but lowest correlation with otehr questions - will be grouped together. This will lead to the 5 factors we have identified at the beginning of the document.

#### Step 4. Develop a scree plot and decide on the number of factors to be derived

We decided to use 5 factors. The decision is based on the following three criteria:
1) eigenvalue > 1
...
colnames(Rotated_Factors)<-paste("Component",1:ncol(UnRotated_Factors),sep=" ")
...
As mentioned at the beginning of these report, the questions that the factors analysis has grouped together refer to 5 themes, broadly speaking:
...
With factors 1 and 2 being the ones with the strongest 'descriptive power' as these two factors have larger eigenvalues.
...
We are pleased with the selection of factors and with the themes that each factor identifies, sow we will proceed with the Segmentation analalysis.
